...
\pertenenciaCoDirector{UNS-CONICET}

% Nombre del cliente, quien va a aprobar los resultados del proyecto, se puede usar con el comando \clientename y \empclientename
\cliente{Mg. Ing. Luciano Tourn }
\empresaCliente{Wúru}
...
3      & Se completa el plan	& 31 de julio de 2025 \\ \hline
...
Por medio de la presente se acuerda con el \authorname\hspace{1px} que su Trabajo Final de la \degreename\hspace{1px} se titulará ``\ttitle'' y consistirá en la creación de un modelo de una aplicación basada en inteligencia artificial para la identificación de eventos en quirófanos por medio de visión por computadora. El trabajo tendrá un presupuesto preliminar estimado de 700 horas y un costo estimado de USD 40075, con fecha de inicio el \fechaInicioName\hspace{1px} y fecha de presentación pública en el mes de  \fechaFinalName.
...
\label{sec:gantt}
En base a las tareas identificadas y a su orden de ejecución (descripto en el diagrama \textit{Activity on Node}), se confeccionó el diagrama de Gantt del proyecto. Para una mejor lectura, en la tabla \ref{tab:wbs} se enumeran las actividades a ser realizadas, junto con sus fechas de comienzo y finalización.  En la figura \ref{fig:gantt_chart} se observa el diagrama de Gantt propiamente dicho.
%\begin{table}[ht]
%	%\caption{Identificación de los interesados}
%	%\label{tab:interesados}
%	\begin{tabularx}{\linewidth}{@{}|l|X|X|l|@{}}
...
\begin{table}[ht]
	\begin{tabularx}{\linewidth}{@{}|l|X|X|l|@{}}
		...
		1.4 Selección y prueba de la herramienta de etiquetado. & 02/07/25 & 15/07/25 \\ \hline
		\multicolumn{3}{|l|}{\textbf{2. Preparación de datos y etiquetado.}} \\ \hline
		2.1 Análisis de videos e identificación de eventos. & 16/07/25 & 04/08/25 \\ \hline
		...
		3.2 Configuración de la herramienta MLflow. & 18/07/25 & 22/07/25 \\ \hline
...
A continuación se detalla el presupuesto del proyecto con sus costos expresados en dólares estadounidenses (1 USD = 1380 ARS).
...
Riesgo 2: los recursos computacionales (GPU) disponibles son insuficientes para completar el entrenamiento y el ensayo de algoritmos en tiempo y forma.
\begin{itemize}
	\item Severidad (S): 6. \\
	Si ocurriera esta situación, el entrenamiento y ensayo de modelos de visión por computadora tomaría más tiempo que el planificado.
	\item Probabilidad de ocurrencia (O): 4. \\
	En el supuesto que la computadora adquirida no fuera suficiente, se negociarán con el cliente opciones de procesamiento en la nube.  %a fin de evitar demoras en las tareas de entrenamiento.
\end{itemize}
...
\begin{itemize}
	\item Severidad (S): 6.\\
	El cronograma del proyecto incluye suficiente holgura en las tareas para acomodar imprevistos en la ejecución y reducir la presión sobre el camino crítico.
...
\begin{table}[htpb]
	...
	\begin{tabularx}{\linewidth}{@{}|X|c|c|c|c|c|c|@{}}
		...
		Los recursos computacionales (GPU) disponibles son insuficientes para completar el entrenamiento y el ensayo de algoritmos en tiempo y forma.   & 6  & 4  &  24   & -   &  -  &  -    \\ \hline
...
Riesgo 1: se acordó con el cliente que se trabajará con datos que abarcan 2 semanas de grabación en un hospital, con posibilidad de expandir la cantidad de ejemplos en caso de ser necesario.
\begin{itemize}
	\item Severidad (S): 8.\\
...
\begin{itemize}
	...
	\item Req \#1.1: el modelo deberá reconocer 5 posibles estados del quirófano: quirófano vacío, entrada de paciente, operación en curso, salida de paciente y quirófano en limpieza.
	...
	\begin{itemize}
		...
		\item Validación: el cliente proporcionará un conjunto de videos nuevos (no usados en entrenamiento/prueba) de quirófanos reales. Se procesarán estos videos con la solución final y el cliente inspeccionará visualmente los resultados en la interfaz de usuario, para confirmar que el modelo identifica correctamente los diferentes estados en escenarios operativos.
	\end{itemize}
	...
	\begin{itemize}
		\item Verificación: se realizarán pruebas \textit{end-to-end} donde se carguen diferentes videos a través de la interfaz de usuario y se monitoree la ejecución del procesamiento hasta la obtención de resultados. 
		\item Validación: el cliente cargará y procesará varios videos de prueba a través de la interfaz de usuario y confirmará que la funcionalidad de procesamiento a demanda es intuitiva y funciona como se espera.
	\end{itemize}
	
	\item Req \#1.4: la solución propuesta deberá indicar el grado de confianza de la detección del estado.
	\begin{itemize}
		\item Verificación: se ejecutará el modelo con un conjunto de videos de prueba y se inspeccionará la salida del modelo para confirmar que, además del estado detectado, se proporciona un valor numérico o una indicación del grado de confianza de la detección.
		\item Validación: el cliente revisará los resultados de detección para varios videos, prestando atención al indicador de confianza. Se discutirá si la representación de esta métrica es clara y útil para la toma de decisiones.
	\end{itemize}
	...
	\begin{itemize}
		\item Verificación: se revisarán los protocolos de anonimización o seudonimización aplicados a los datos de video y a cualquier metadato sensible. Se verificará que el acceso a los datos crudos está estrictamente controlado y que no se almacena información personal identificable sin consentimiento explícito. Se realizará una auditoría del flujo de datos para identificar posibles puntos de fuga.
		\item Validación: el cliente (o un representante de seguridad de datos del hospital) revisará la política de manejo de datos y los procedimientos implementados, para confirmar que cumplen con las normativas de privacidad aplicables y sus propias expectativas de confidencialidad.
	\end{itemize}
	...
	\begin{itemize}
		...
		\item Validación: el cliente verificará que el repositorio de código es privado y que solo las personas designadas tienen acceso, lo que comprueba que la confidencialidad del código fuente está asegurada.
	\end{itemize}
	...
	\begin{itemize}
		...
		\item Validación: el director del proyecto, el equipo docente, y los jurados (para la memoria final) revisarán los documentos, proporcionarán retroalimentación y aprobarán su contenido y formato. 
	\end{itemize}
	...
	\begin{itemize}
		\item Verificación: se realizarán pruebas de usabilidad internas para asegurar que la interfaz es intuitiva, que los elementos están bien organizados y que la función de carga de video es fácil de encontrar y usar. Se verificarán diferentes escenarios de carga (por ejemplo, con distintos formatos de archivo, diversos tamaños, etc.).
		\item Validación: el cliente interactuará con la interfaz de usuario para cargar y procesar videos, y proporcionará retroalimentación sobre la simplicidad, claridad y facilidad de uso de la función de carga de videos.
	\end{itemize}
	...
	\begin{itemize}
		...
		\item Validación: el cliente observará la interfaz de usuario mientras se procesan videos y confirmará que el estado del quirófano se muestra de manera clara, precisa y útil. %para su monitoreo.
	\end{itemize}
...
\begin{itemize}
	\item Actividades de cierre con el cliente:
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item Transición del repositorio de código al cliente, entrega de la documentación asociada y de los hallazgos del proyecto.
			\item Repaso de recomendaciones para la implementación en producción, de las modificaciones a la arquitectura implementada (si las hubiere) y de los detalles del modelo desarrollado.
		\end{itemize}
	\end{itemize}
		%\item Análisis de las pautas de trabajo:
		\item Defensa pública del trabajo:
	\begin{itemize}
		\item[] Responsable: \authorname
		\item[] Procedimiento:
		\begin{itemize}
